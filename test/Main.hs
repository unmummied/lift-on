...
prop_on5 (Fun _ q') x y z w v (Fun _ f) = on5 q x y z w v f `shouldBe` q (f x) (f y) (f z) (f w) (f v) where q = curry5 q'
...
prop_on5Id (Fun _ q') x y z w v = on5 q x y z w v id `shouldBe` q x y z w v where q = curry5 q'
